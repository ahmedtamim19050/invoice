        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($invoices as $invoice)
                <article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-sky-200 hover:shadow-md">
                    ...
                    <div class="grid grid-cols-3 gap-2 border-t border-slate-100 bg-slate-50/50 px-5 py-3">
                        <a href="{{ route('invoices.show', $invoice) }}" class="rounded-lg bg-white px-2 py-2 text-center text-xs font-semibold text-slate-700 ring-1 ring-slate-200 transition hover:bg-slate-50 sm:px-3 sm:text-sm">
                            View
                        </a>
                        <a href="{{ route('invoices.download', $invoice) }}" class="rounded-lg bg-emerald-600 px-2 py-2 text-center text-xs font-semibold text-white transition hover:bg-emerald-500 sm:px-3 sm:text-sm">
                            Download
                        </a>
                        <a href="{{ route('invoices.print', $invoice) }}" target="_blank" class="rounded-lg bg-sky-600 px-2 py-2 text-center text-xs font-semibold text-white transition hover:bg-sky-500 sm:px-3 sm:text-sm">
                            Print
                        </a>
                    </div>
                    ...
                    <div class="border-t border-slate-100 bg-white px-5 py-3">
                        @include('invoices.partials.delete-button', [
                            ...
                            'class' => 'delete-invoice-btn flex w-full items-center justify-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-600 transition hover:bg-red-100 sm:text-sm',
                        ])
                    </div>
                </article>
            @endforeach
