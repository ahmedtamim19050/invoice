...
<form id="invoice-form" method="POST" action="{{ $action }}" class="space-y-6">
    @csrf
    @if ($isEdit)
        @method('PUT')
    @endif
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="sm:col-span-2">
                ...
                <input type="text" name="recipient_name" value="{{ old('recipient_name', $invoice->recipient_name ?? '') }}" required
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="recipient_address" rows="2" required
                    class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
                    placeholder="22ka topkhana Road, Azmiri complex 2nd floor, Dhaka-1000.">{{ old('recipient_address', $invoice->recipient_address ?? '') }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="recipient_contact" value="{{ old('recipient_contact', $invoice->recipient_contact ?? '') }}" required
                    class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
            ...
            <div>
                ...
                <input type="text" name="subject" value="{{ old('subject', $invoice->subject ?? '') }}" required
                    class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
        <a href="{{ $cancelUrl }}" class="rounded-xl border border-slate-200 bg-white px-6 py-3 text-center text-sm font-semibold text-slate-700 hover:bg-slate-50">
            Cancel
        </a>
...
@if (! empty($initialItems))
    <script type="application/json" id="invoice-initial-items">@json($initialItems)</script>
@endif
